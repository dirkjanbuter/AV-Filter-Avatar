...
void pixelblend(int x, int y, int color)
{
    ...
    _oi[i+0] = (_oi[i] * ininva + inr * ina) >> 8;
...
void lineblend(VECTOR2D *v1, VECTOR2D *v2, int color)
{
    ...
    for (int x = x0; x != x1; x += xstep) {
        ...
        // plot
        color?whitepixel(xDraw, yDraw):redpixel(xDraw, yDraw);
...
void cuberender(int wireframe) {
    ...
    for(i = 0; i < 36; i += 3)
    {
        ...
        if(wireframe) {
            lineblend(t1, t2, _talk);
            lineblend(t2, t3, _talk);
            lineblend(t1, t3, _talk);
        }
        //else
...
int filterstep(unsigned char *buffer, int w, int h, unsigned int color, char *text, int64_t framecount)
{
    ...
    _scale = matrix3d_scale(_scalefactor, _scalefactor, _scalefactor); 	
